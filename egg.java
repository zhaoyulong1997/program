...
public class egg {
     ...
     */
    public int dropEggs2(int K, int N) {
        ...
        for (int egg=2;egg<=K;egg++) {
            ...
            for (int floor=2;floor<=N;floor++) {
                ...
                for (int drop=1;drop<=floor;drop++) {
                    ...
                    //第一种情况，哎呀~碎了！那么剩下的问题就转化成了如何在drop-1层(层数），用egg-1个鸡蛋寻找最优解
                    int broken = state[egg - 1][drop - 1];
                    //第二种请看，卧槽~没碎！问题就转化成了如果再floos-drop层（层数），用egg个鸡蛋寻找最优解
